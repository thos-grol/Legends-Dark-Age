...
function start()
{
	...
	// 	this.Flags.set("Bribe", this.Contract.beautifyNumber(this.Contract.m.Payment.getOnCompletion() * ::Math.rand(100, 300) * 0.01));
	// }
	// else
	// {
...
function createStates()
{
	...
	this.m.States.push({
		...
		{
			if (_combatID == "Event_Return_Item")
			{
				::Z.T.Contract.build_stack_combat(true);
			}
		}
		...
		function onRetreatedFromCombat( _combatID )
		{
			if (_combatID == "Event_Return_Item")
			{
				::Z.T.Contract.build_stack_combat(false);
			}
		}
	});
}
...
function createScreens()
{
	...
	this.importScreens(::Const.Contracts.Overview_NEW);
	this.m.Screens.push({
		ID = "Task",
		Title = "",
		...
		Options = [
			{
				...
				function getResult()
				{
					::Z.T.Contract.Stack.push("Bandits");
					return "Negotiation";
				}
			},
			{
	...
	this.m.Screens.push({
		...
		Options = [
			{
				...
				function getResult()
				{
					//setup outcomes - see running state for evaluation
					::Z.T.Contract.set_outcomes_combat("Success1", "Failure1");
					::World.State.startScriptedCombat(this.Contract.m.Combat_Properties, false, false, true);
					return 0;
				}
		...
		function start()
		{
			...
			::Const.World.Common.addUnitsToCombat(
				...
				80 * this.Contract.getDifficultyMult() * this.Contract.getScaledDifficultyMult(), 
				::World.FactionManager.getFactionOfType(::Const.FactionType.Bandits).getID()
			);

			this.Contract.m.Combat_Properties = properties;
			::Z.S.show_enemy_list(properties.Entities, this.List);
		}
	});
	// this.m.Screens.push({
	// 	ID = "Necromancer",
		...
		Options = [
			{
				...
				function getResult()
				{
					::World.Assets.addMoney(this.Flags.get("Bribe"));
					::World.Assets.addBusinessReputation(::Const.World.Assets.ReputationOnContractBetrayal);
					::World.FactionManager.getFaction(this.Contract.getFaction()).addPlayerRelation(::Const.World.Assets.RelationCivilianContractFail, "Failed to return stolen " + this.Flags.get("Item"));
		...
		Options = [
			{
				...
				function getResult()
				{
					...
					::World.FactionManager.getFaction(this.Contract.getFaction()).addPlayerRelation(::Const.World.Assets.RelationCivilianContractSuccess, "Returned stolen " + this.Flags.get("Item"));
