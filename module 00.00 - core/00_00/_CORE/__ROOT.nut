// !! For core core stuff, see scripts/config

::Z.S <- {}; // S for static
::Z.T <- {}; // T for temp

::DEF.C <- {}
::DEF.C.Factions <- {}
::DEF.C.Factions.Decks <- {};

::Z.Log <- {};
::Z.Lib <- {};

::B <- {}; // B for build

///////////////

// ::Z.Economy <- {};
// ::Z.Backgrounds <- {};

// ::DEF.Perks <- {};


// ::B <- {}; //B for builds
// ::B <- {};
// ::B.Lib <- {};

// ::Z.Log.HasActed <- false;


// //Item Rarity

// ::DEF.Color.Common <- "#666666";
// ::DEF.Color.Uncommon <- "#246f45";
// ::DEF.Color.Rare <- "#26619C";
// ::DEF.Color.Legendary <- "#ab3f17";
// ::DEF.Color.Mythic <- "#38639b";


// /////////////////

// ::Const.Combat.OpportunistUsedTiles <- [];

// ::Z.S.TraininableProficiencies <- [
//     "trait.proficiency_Axe",
//     "trait.proficiency_Cleaver",
//     "trait.proficiency_Sword",
//     "trait.proficiency_Mace",
//     "trait.proficiency_Hammer",
//     "trait.proficiency_Flail",
//     "trait.proficiency_Spear",
//     "trait.proficiency_Polearm",
//     "trait.proficiency_Fist",
//     "trait.proficiency_Ranged",
// ];

// ::Z.S.TraininableAttributes <- [
//     "trainable_hitpoints",
//     "trainable_resolve",
//     "trainable_fatigue",
//     "trainable_initiative",
//     "trainable_meleeskill",
//     "trainable_meleedefense",
//     "trainable_rangedskill",
//     "trainable_rangeddefense",
// ];

// /////////////////

